          ...
          questions: [
            {
              ...
              choices: [
                'a single linear function, no matter how many layers it has',
                'a more flexible function than any 2-layer network',
                'nothing — it cannot produce an output',
                'a nonlinear function, because depth itself adds curvature',
              ],
              answer: 0,
              explain:
                'Linear ∘ linear = linear. Composing linear maps just multiplies matrices into one matrix — the nonlinear activations are what give depth its power.',
            },
            {
              kind: 'numeric',
              id: 'ch06-q-nn-2',
            ...
            },
            {
              ...
              answer: 0,
              explain:
                'Sigmoid squashes the output into (0, 1), readable as the probability of the positive class — linear output would make it a regression model instead.',
            },
          ],
        },
      ],
          ...
          questions: [
            {
              ...
              choices: [
                'an efficient chain-rule scheme for computing the gradient of the cost with respect to every parameter',
                'a way to run the network backwards to generate inputs',
                'a rule for choosing the number of hidden units',
                'a replacement for gradient descent',
              ],
              answer: 0,
              explain:
                'Backprop is “just” the chain rule, organized cleverly: the output error flows backwards, and each layer reuses the work of the layer after it. Gradient descent then updates the parameters.',
            },
            {
              kind: 'tf',
              id: 'ch06-q-tinynet-2',
            ...
            },
            {
              ...
              choices: [
                'raises the network’s capacity, letting the boundary bend in more places',
                'makes each training epoch update the weights by a larger step',
                'changes the number of input features',
                'guarantees convergence to 100% accuracy',
              ],
              answer: 0,
              explain:
                'More hidden units = more learned line-detectors to combine = a more flexible boundary. It buys flexibility, not a convergence guarantee — bad starts can still stall.',
            },
          ],
        },
      ],
          ...
          questions: [
            {
              ...
              choices: [
                'one small filter’s weights are reused at every position of the image',
                'CNNs work on smaller images',
                'convolution layers have no weights at all',
                'CNNs skip the nonlinear activations',
              ],
              answer: 0,
              explain:
                'Weight sharing: a 3×3 filter costs 9 weights however large the image is, because the same detector slides everywhere. An MLP would pay for every pixel-to-unit wire.',
            },
            {
              kind: 'numeric',
              id: 'ch06-q-cnn-2',
            ...
            },
            {
              ...
              choices: ['smaller — the window skips positions', 'larger', 'identical in size', 'all zeros'],
              answer: 0,
              explain: 'Stride is the step size of the moving window; bigger steps mean fewer stops, hence a smaller map. Padding is the knob that makes it bigger.',
            },
            {
              kind: 'tf',
              id: 'ch06-q-cnn-4',
        ...
        },
        {
          ...
          questions: [
            ...
            },
            {
              ...
              choices: [
                'unrolling makes the network very deep, and repeated small derivatives shrink early gradients toward zero',
                'the state vector physically runs out of storage',
                'the weights change at every timestep',
                'backprop cannot be applied to sequences at all',
              ],
              answer: 0,
              explain:
                'Backprop through time multiplies one small factor per step; over many steps the product vanishes, so early inputs barely influence learning — and get “forgotten”.',
            },
            {
              kind: 'mcq',
              id: 'ch06-q-rnn-4',
              ...
              choices: [
                'when the unit’s memory cell is written, kept, or erased',
                'the learning rate of gradient descent',
                'the number of timesteps in the input',
                'which activation function each layer uses',
              ],
              answer: 0,
              explain:
  ...
  bossPool: [
    {
      ...
      choices: [
        ...
        'a sum of three independent models: $f_1 + f_2 + f_3$',
        'three models voting by majority',
        'a lookup table with three columns',
      ],
      answer: 0,
      explain: 'Layers compose: each layer’s output vector becomes the next layer’s input. That nesting is the defining shape of a feed-forward network.',
    },
    {
      kind: 'mcq',
      id: 'ch06-boss-2',
      ...
      choices: [
        'apply a fixed nonlinearity so the network can approximate nonlinear functions',
        'normalize the data to zero mean',
        'select which units get deleted',
        'compute the gradient of the cost',
      ],
      answer: 0,
      explain: 'The activation is chosen before training and stays fixed; the learnable parts are W and b. Without it, stacking layers would buy nothing.',
    },
    {
      kind: 'tf',
      id: 'ch06-boss-3',
    ...
    },
    {
      ...
      choices: [
        'each of its units receives the outputs of every unit in the previous layer',
        'it is connected to both the input and the output layer',
        'its weights are all nonzero',
        'it contains the maximum possible number of units',
      ],
      answer: 0,
      explain: 'Full connectivity is about the wiring pattern between consecutive layers — every output feeds every unit. CNNs exist precisely to relax this.',
    },
    {
      kind: 'mcq',
      id: 'ch06-boss-7',
    ...
    },
    {
      ...
      choices: [
        'computing the gradients of the cost for all parameters efficiently via the chain rule',
        'propagating training examples backwards through the dataset',
        'removing layers that are not needed',
        'replacing gradient descent with a closed-form solution',
      ],
      answer: 0,
      explain: 'It is the chain rule organized layer by layer, reusing intermediate results — the gradients then feed ordinary gradient descent.',
    },
    {
      kind: 'mcq',
      id: 'ch06-boss-9',
      ...
      choices: [
        'training neural networks with the modern algorithmic toolkit, more or less independently of depth',
        'strictly networks with at least ten layers',
    ...
    },
    {
      ...
      choices: [
        'ReLU activations, gated units (LSTM), and skip connections in residual networks',
        'bigger images, more classes, and longer training',
        'decision trees, kernels, and margins',
        'bag of words, one-hot encoding, and normalization',
      ],
      answer: 0,
      explain: 'Each attacks the vanishing gradient from a different side: ReLU avoids tiny derivatives, gates preserve memory, skip connections shorten gradient paths.',
    },
    {
      kind: 'numeric',
      id: 'ch06-boss-12',
    ...
    },
    {
      ...
      choices: [
        'the patch’s pattern resembles the pattern stored in the filter',
        'the patch contains mostly zeros',
        'the filter is larger than the patch',
        'the stride is set to 2',
      ],
      answer: 0,
      explain: 'It is a dot product: big when large filter weights line up with bright pixels. That is what makes a filter a pattern detector.',
    },
    {
      kind: 'match',
      id: 'ch06-boss-14',
    ...
    },
    {
      ...
      choices: [
        'the stack of all feature maps from the first layer — a volume — and convolves through its whole depth',
        'only the single best feature map',
        'the raw image again',
        'the transposed filters of the first layer',
      ],
      answer: 0,
      explain: 'Layer l outputs one map per filter; layer l+1 treats that stack as a volume. This is how “patterns of patterns” — corners, textures, objects — emerge.',
    },
    {
      kind: 'mcq',
      id: 'ch06-boss-17',
      ...
      choices: [
        'its units keep a state that is fed back in at the next timestep — the network contains loops',
        'it has more layers',
        'it can only handle images',
        'it trains without gradient descent',
      ],
      answer: 0,
      explain: 'The state vector is the unit’s memory of everything read so far; feeding it back is the loop that “recurrent” refers to.',
    },
    {
      kind: 'order',
      id: 'ch06-boss-18',
    ...
    },
    {
      ...
      choices: [
        'longer input sequences unroll into deeper networks, multiplying more small derivatives together',
        'RNNs have no biases',
        'the state vector grows with every timestep',
        'sequences cannot be differentiated',
      ],
      answer: 0,
      explain: 'Each timestep is one more layer in the unrolled chain — a 100-word sentence means a 100-deep gradient path of sub-1 factors.',
    },
    {
      kind: 'mcq',
      id: 'ch06-boss-20',
      ...
      choices: [
        'a memory cell that is kept unchanged acts like the identity function, whose derivative is constant',
        'they use much larger learning rates',
        'they skip the backpropagation step entirely',
        'their gates delete the oldest timesteps',
      ],
      answer: 0,
      explain: 'Storing a value untouched is applying f(x) = x; its derivative is 1, so the gradient passes through those timesteps without shrinking.',
    },
    {
      kind: 'tf',
      id: 'ch06-boss-21',
