...
export const conceptsCh07: Concept[] = [
  ...
  },
  {
    ...
    term: 'bandwidth',
    simple:
      'How far each training point’s influence reaches. Narrow it and the model listens only to its immediate neighbours; widen it and distant points get a say too, which calms the curve down.',
    technical:
  ...
  },
  {
    ...
    simple:
      'Read a whole sentence, work out what it means, then write a new one that need not be the same length. Four English words in, three French words out — a translator, a summariser, a chatbot.',
    technical:
  ...
  },
  {
    ...
    technical:
      'A learned, dense, low-dimensional representation, as opposed to a sparse [[one-hot-encoding|one-hot]] code, which is orthogonal to everything and therefore says nothing about similarity. What makes embeddings valuable is that they *transfer*: a vector produced by an encoder trained on one task can be compared with [[cosine-similarity]], clustered, indexed for nearest-neighbour search, or fed to a second model trained on something else entirely. Chapter 7 uses them three ways — as the handoff between encoder and decoder, as the face vector in a [[siamese-network]], and as the label vector that makes [[zero-shot-learning]] possible.',
    math:
      'A map $\\phi: \\mathcal{X}\\to\\mathbb{R}^d$ with $d$ far below the number of distinct inputs — 300 dimensions for a vocabulary of 100,000 words. Similarity is read as $\\frac{\\phi(a)\\cdot\\phi(b)}{\\|\\phi(a)\\|\\,\\|\\phi(b)\\|}$ or as squared distance $\\|\\phi(a)-\\phi(b)\\|^2$; on unit-normalized vectors $\\|\\phi(a)-\\phi(b)\\|^2 = 2 - 2\\cos\\theta$, so the two rank neighbours identically.',
  ...
  },
  {
    ...
    technical:
      'The move is to stop predicting a class index and start predicting a point in a semantic space that the classes also live in: [[word-embeddings]], or hand-written attribute vectors. A [[softmax]] cannot do this at all, because its output units are fixed the moment training starts and an unseen class has no unit to light up. Prediction becomes nearest-neighbour search over whatever label vocabulary you supply at test time, and that vocabulary may grow long after training. Two known weaknesses: hubness, where a few semantic vectors turn out to be the nearest neighbour of almost everything, and a strong pull toward the classes actually seen in training.',
    math:
