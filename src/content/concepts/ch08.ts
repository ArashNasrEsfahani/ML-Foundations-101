...
export const conceptsCh08: Concept[] = [
  ...
  },
  {
    ...
    technical:
      'Each example’s loss is multiplied by a factor attached to its class, so the optimiser will trade several cheap majority mistakes for one expensive minority catch and the boundary moves accordingly. Cheaper than resampling — no rows are copied or discarded, so training time is unchanged — and supported directly by most SVM, tree and neural implementations. The library default of weights inversely proportional to class frequency is a starting point, not an answer: the honest weight comes from what a false negative actually costs, and a missed tumour and a missed advert are not the same number.',
    math:
  ...
  },
  {
    ...
    math:
      'Duplicating a minority example $r$ times multiplies its total loss contribution by $r$, exactly as a class weight of $r$ would. What differs is everything downstream of the duplicates: a [[bootstrap-sample]] now draws copies of copies, and [[k-nearest-neighbors|kNN]] finds $k$ identical points at distance 0, so the effective neighbourhood shrinks to a single original example.',
    teachesAt: 'ch08-imbalanced',
  ...
  },
  {
    ...
    technical:
      'Adaptive Synthetic sampling. Each minority point receives a quota proportional to how many of its neighbours belong to the majority class, so a point surrounded by the enemy is judged hard and gets many synthetics, while a point buried inside a minority cluster gets almost none. The effect is to push the decision boundary outward into the difficult region rather than fattening the easy one. That same instinct is the weakness: noise and mislabelled minority points look exactly like hard frontier points from the inside, so ADASYN amplifies them harder than [[smote]] does.',
    math:
      'For minority point $\\mathbf{x}_i$ let $r_i$ be the fraction of its $k$ nearest neighbours that belong to the majority class; normalize $\\hat{r}_i = r_i / \\sum_j r_j$ and generate $g_i = \\hat{r}_i \\cdot G$ synthetics at $\\mathbf{x}_i$, where $G$ is the total wanted. Each one is built by the same interpolation SMOTE uses. Setting every $\\hat{r}_i$ equal recovers plain SMOTE, so ADASYN is SMOTE with a non-uniform budget.',
    teachesAt: 'ch08-imbalanced',
