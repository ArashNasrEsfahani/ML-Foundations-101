...
export const conceptsCh10: Concept[] = [
  ...
  },
  {
    ...
    technical:
      'Three axioms — nonnegativity (zero only for identical points), symmetry, and the [[triangle-inequality]]. The axioms are not pedantry. Nearest-neighbor indexes, ball trees and a good deal of clustering theory use the triangle inequality to prove that an entire branch of the search cannot contain the answer and skip it without computing anything; hand them a similarity that violates it and they return wrong neighbors rather than slow ones. [[cosine-similarity|Cosine distance]] is the everyday example of something useful that is not a metric. Since the choice changes the output of every neighbor-based algorithm you own, [[metric-learning|learning it]] is a reasonable thing to want.',
    math:
  ...
  },
  {
    ...
    technical:
      'Supervised learning where one training example is an entire labeled list — a query with its candidate documents, each a feature vector, each with a relevance judgement. Only the ordering is graded, so a model that doubles every score is exactly as good as the original. Three framings differ in how much of the list a single training signal sees: [[pointwise-ranking|pointwise]], [[pairwise-ranking|pairwise]] and [[listwise-ranking|listwise]]. Beyond web search the same machinery drives feeds, ad auctions and the re-ranking stage of a recommender, where a cheap model fetches a thousand candidates and this one decides the order of the ten that get shown.',
    math:
  ...
  },
  {
    ...
    math:
      'Model $\\Pr(i \\succ k) = \\sigma\\!\\left(h(\\mathbf{x}_i) - h(\\mathbf{x}_k)\\right)$ and fit with cross-entropy against the labeled order — RankNet’s formulation. Because only the *difference* of scores enters, $h$ is identified up to an additive constant, and the sort is invariant to it. Sorting with a comparator costs $O(m\\log m)$ comparisons rather than the $O(m^2)$ pairs used in training.',
    teachesAt: 'ch10-ranking',
  ...
  },
  {
    ...
    technical:
      'Supervision manufactured from the input by a *pretext task*: predict the neighboring words, fill in a masked token, restore a corrupted image, judge whether two crops came from the same photo. Since nobody annotates anything, the training set is as large as the internet, and the representation learned along the way transfers to real tasks with a fraction of the labeled data — this is the engine behind word2vec, BERT and every modern language model. It sits between [[supervised-learning|supervised]] and [[unsupervised-learning|unsupervised]] learning: the machinery is supervised, the data is not. Designing the pretext task is the whole art, since one too easy teaches nothing and one impossible teaches noise.',
    math:
      'Given unlabeled $\\{\\mathbf{x}_i\\}$, choose a transformation splitting each example into an input and a target, $(\\tilde{\\mathbf{x}}_i, t_i) = T(\\mathbf{x}_i)$, then minimize $\\sum_i \\ell\\!\\left(f(\\tilde{\\mathbf{x}}_i),\\, t_i\\right)$ by ordinary supervised means. For [[skip-gram]], $T$ takes a window and returns (center, context); for a masked language model, $T$ blanks 15% of the tokens and returns them as the target.',
    teachesAt: 'ch10-embeddings',
